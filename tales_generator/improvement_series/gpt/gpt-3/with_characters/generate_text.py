...
objects = ["девочка", "папа", "мама", "собака", "кот"]
backup = ["Колобок", "Баба", "Яга", "Буратино", "Золушка", "Винни-Пух",
          "Чебурашка", "Щелкунчик", "Муха-Цокотуха", "Принцесса", "Кощей",
          "Алладин", "Царевна", "Емеля", "Русалочка", "Мойдодыр", "Леший",
          "Дюймовочка", "Белоснежка", "Маугли", "Снегурочка", "Принц",
          "Водяной", "Незнайка", "Мальвина", "Иван-дурак", "Морозко"]
stack = []
    ...
    if stack:
        ...
        if check:
            ...
            try:
                ...
                name = character
                #name = morph.parse(character)[0].inflect({word[3],
                # word[4][4:]})[0]
            except IndexError:
                name = character
            return name
            ...
            if morph.parse(candidate)[0].tag.gender == word[2]:
                try:
                    ...
                    name = candidate
                    #name = morph.parse(candidate)[0].inflect({word[3],
                    # word[4][4:]})[0]
                except IndexError:
                    name = candidate
                ...
                if any(morph.parse(element)[0].tag.gender == word[2] for \
        ...
        if morph.parse(candidate)[0].tag.gender == word[2]:
            ...
            if any(morph.parse(element)[0].tag.gender == word[2] for element \
                   in characters):
                name = choose_character(word, characters=objects)
            else:
                name = choose_character(word, characters=backup)
...
def generate(prompt, length=500):
    model_name_or_path = r"C:\Users\Eugene\PycharmProjects\tales_generator\experiments\improvement_series\GPT-3\with_characters\model\1_epoch"
    stop_token = "<end>"
...
def make_readable_story(prompt):
    ...
    ending = generate("<ending>" + temp, 200)
    output = main_part + ending[:ending.rfind(".") + 1]
    output = embed_characters(make_replaces(output))
    return output


prompt = "В тридевятом царстве, в тридесятом государстве жил-был старик со старухою."
